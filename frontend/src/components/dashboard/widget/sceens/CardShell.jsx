...
export default function CardShell({
  ...
  const { cardClasses, contentClasses, extraPaddingClasses } = useMemo(() => {
    ...
    return {
      cardClasses:
        "relative bg-white rounded-[15px] shadow-[0_2px_15px_rgba(0,0,0,0.15)] min-h-0 z-20 flex flex-col",
      contentClasses: `p-4 sm:p-6 lg:p-8 ${leftPad} min-h-0 flex flex-col flex-1 overflow-hidden`,
  ...
  return (
    <div className={cardClasses}>
      ...
      <div className={contentClasses}>
        <div className={`${extraPaddingClasses} flex flex-col flex-1 overflow-hidden`}>{children}</div>
      </div>
    </div>
  );
}
