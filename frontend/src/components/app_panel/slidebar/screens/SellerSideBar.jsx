...
import { sellerMenu } from "../lib/sellerMenu";
import { Menu, X, ChevronRight, ChevronDown, Package } from "lucide-react";
import { useTranslation } from "react-i18next";

export default function SellerSideBar({ active, onSelect, isOpen, onToggle }) {
    const { t } = useTranslation();
    const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
    const menuRef = useRef(null);

    // Get active item info
    const activeItem = sellerMenu.find(it => it.label === active);
            ...
            <div className="md:hidden relative p-3" ref={menuRef}>
                ...
                >
                    <div className="flex items-center gap-2">
                        ...
                        <span>{t(activeItem?.transKey) || activeItem?.label || t("seller_menu")}</span>
                    </div>
